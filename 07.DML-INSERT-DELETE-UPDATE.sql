/*
DML(Data Maninpulation Language)
*/
...
*/
--primary key가 중복된경우
insert into dept(deptno,dname,loc) values(96,'마케팅1과','제주');
--ORA-00001: unique constraint (SCOTT.PK_DEPT) violated

--primary key가 null인경우
insert into dept(deptno,dname,loc) values(null,'마케팅2과','충추');
...
insert into dept(deptno,dname,loc) values(97,'미래비젼팀','CA');--한글1글자당(3바이트)
--ORA-12899: value too large for column "SCOTT"."DEPT"."DNAME" (actual: 15, maximum: 14)

--데이터 타입이 일치하지 않는경우
insert into dept(deptno,dname,loc) values('구십칠','기획팀','광주');
--ORA-01722: invalid number

insert into dept(deptno,dname,loc) values(97,50000,'대구');
--숫자는 문자화되서 삽입된다.
commit;
...
--ORA-02291: integrity constraint (SCOTT.FK_DEPTNO) violated - parent key not found-무결성제약조건
--insert into emp values(9005,'김수한',null,null,null,null,null,99);
...
--1 행 이(가) 업데이트되었습니다.
update dept set loc='서울' where loc is null;
--4개 행 이(가) 업데이트되었습니다.
update dept set dname='영업' where dname is null;

--update dept set loc='SEOUL근교' where loc='서울';
