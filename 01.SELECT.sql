...
select 60*60*24*365|| '초' "일년을 초로환산" from dual;
...
select empno, enmae, sal*12 from emp order by sal*12 desc;
select empno, ename, sal*12 연봉 from emp order by 연봉 desc; --alias
select empno, ename, sal*12 연봉 from emp order by 3 desc;    --컬럼순서번호

--문자정렬
select empno, ename, sal from emp order by emane asc;
select empno, ename, sal from emp order by emane desc;
--날짜정렬(현재와가까운시간이 더크다)
